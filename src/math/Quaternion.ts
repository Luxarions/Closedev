...
/**
 * Class for representing a Quaternion in 3D space / rotations.
 */
export class Quaternion {
  ...
  static slerpFlat(
    dst: number[] | Float32Array,
    dstOffset: number,
    src0: number[] | Float32Array,
    srcOffset0: number,
    src1: number[] | Float32Array,
    srcOffset1: number,
    t: number
  ): void {
  ...
  setFromEuler(euler: { _x: number; _y: number; _z: number; _order: string }, update = true): this {
    const x = euler._x,
      y = euler._y,
      z = euler._z,
      order = euler._order;
  ...
  setFromRotationMatrix(m: { elements: number[] | Float32Array }): this {
    const te = m.elements,
      m11 = te[0],
      m12 = te[4],
      m13 = te[8],
  ...
  setFromUnitVectors(
    vFrom: { x: number; y: number; z: number; dot(v: any): number },
    vTo: { x: number; y: number; z: number }
  ): this {
